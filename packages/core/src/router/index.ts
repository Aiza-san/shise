...
export class Router {
	...
	async find(method: Method, path: string, params: Record<string, string>, pathnames: string[]) {
		...
		async function validateMethod(
			...
			if (controller instanceof Router) {
				...
				const existsController = await controller.find(method, parts.slice(1).join('/'), params, pathnames)
		...
		async function findController(
			...
			for (const pathname in controllers) {
				...
				pathnames.push(pathname.startsWith(':') ? currentPart : pathname)
